...
class ListScheduleBloc extends Bloc<ListScheduleEvent, ListScheduleState> {
  ...
  Map<String, List<ScheduleItem>> _getMapScheduleItems(DateTime d) {
    ...
    _schedules.forEach((schedule) {
      for (int i = 0;; i++) {
        DateTime current =
            startSemester.add(Duration(days: schedule.dayOfWeek - 1 + i));
        if (current.month < month)
          continue;
        else if ((current.month > month && current.year == d.year) ||
            current.year > d.year) break;
        Duration dif = current.difference(
            startSemester.add(Duration(days: schedule.dayOfWeek - 1)));
        if (dif.inDays % 7 == 0 && schedule.weeks.contains(dif.inDays ~/ 7)) {
          String key = _formatter.format(current);
          if (!res.containsKey(key)) res[key] = [];
          res[key]?.add(ScheduleItem(
              schedule,
              _getStart(schedule.startAt),
              _getEnd(schedule.endAt),
              schedule.startAt,
              schedule.endAt,
              current,
              schedule.classroom,
              schedule.registerableClass.subject.subjectName));
        }
      }
    });
    return res;
