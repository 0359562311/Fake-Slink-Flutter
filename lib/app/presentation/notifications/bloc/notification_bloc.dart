...
class NotificationBloc extends Bloc<NotificationEvent, NotificationState>{
  ...
  NotificationBloc() : super(NotificationLoadingState('General')) {
    ...
    _markNotificationAsReadUseCase = GetIt.instance<MarkNotificationAsReadUseCase>();
  }
  ...
  Stream<NotificationState> mapEventToState(NotificationEvent event) async* {
    if(event is NotificationInitEvent) {
      ...
      final res = await _listNotificationsUseCase.execute(0, event.type);
      ...
      final res = await _listNotificationsUseCase.execute(_notifications[event.type]!.length, event.type);
